...
exports.sendSessionRegistrationPendingEmail = sendSessionRegistrationPendingEmail;
exports.sendSessionRegistrationConfirmedEmail = sendSessionRegistrationConfirmedEmail;
exports.sendSessionRegistrationRejectedEmail = sendSessionRegistrationRejectedEmail;
...
function eventRegistrationHtml(title, intro, data) {
...
async function sendEventRegistrationPendingEmail(data) {
    ...
    await sendMail(data.to, subject, eventRegistrationHtml("Inscription reçue", intro, data), `Bonjour ${data.fullName}, votre inscription à "${data.eventTitle}" a bien été reçue. Elle sera validée par notre équipe.`);
}
...
function sessionRegistrationHtml(title, intro, data) {
    const date = formatDate(data.startDate);
    return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f2340;max-width:620px;margin:0 auto;padding:24px">
      <h1 style="font-size:20px;margin:0 0 16px;color:#0b2545">${escapeHtml(title)}</h1>
...
async function sendSessionRegistrationPendingEmail(data) {
    ...
    await sendMail(data.to, `Demande d'inscription reçue — ${data.sessionTitle}`, sessionRegistrationHtml("Demande d'inscription reçue", intro, data), `Bonjour ${data.fullName}, votre demande d'inscription à "${data.sessionTitle}" a bien été reçue. Notre équipe vous recontactera très prochainement.`);
}
// Envoyé quand l'admin valide l'inscription.
async function sendSessionRegistrationConfirmedEmail(data) {
    ...
    await sendMail(data.to, `Inscription confirmée — ${data.sessionTitle}`, sessionRegistrationHtml("Inscription confirmée", intro, data), `Bonjour ${data.fullName}, votre inscription à "${data.sessionTitle}" est confirmée.`);
}
// Envoyé quand l'admin refuse l'inscription (session complète, profil non retenu…).
async function sendSessionRegistrationRejectedEmail(data) {
    const intro = "Nous vous remercions vivement pour l'intérêt que vous portez à cette formation. " +
    ...
    await sendMail(data.to, `Concernant votre inscription — ${data.sessionTitle}`, sessionRegistrationHtml("Votre inscription n'a pas pu être retenue", intro, data), `Bonjour ${data.fullName}, merci pour votre intérêt pour "${data.sessionTitle}". Nous ne sommes malheureusement pas en mesure de retenir votre inscription pour cette session. Nous serons ravis de vous accueillir lors d'une prochaine session.`);
}
